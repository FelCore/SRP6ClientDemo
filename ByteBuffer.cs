...
namespace SRP6ClientDemo
{
    ...
    public unsafe class ByteBuffer : IDisposable
    {
        ...
        public ByteBuffer(MessageBufferHG buffer)
        {
            _rpos = 0;
            _wpos = 0;
            ...
            buffer.GetReadSpan(_size).CopyTo(new Span<byte>((void*)_storage, _size));
        }
        ...
        public static implicit operator ByteBuffer(MessageBufferHG buffer) => new ByteBuffer(buffer);
        ...
        {
            Debug.Assert(addSize > 0, string.Format("Attempted to put a zero-sized value in ByteBuffer (pos: {0} size: {1})", _wpos, Size()));
        ...
        public void Append(string value, bool withZero = true)
        {
        ...
        {
            Debug.Assert(pos >= 0, string.Format("Attempted to put value with invalid pos: {0} in ByteBuffer", pos));
        ...
        {
            Debug.Assert(pos >= 0, string.Format("Attempted to put value with invalid pos: {0} in ByteBuffer", pos));
            Debug.Assert(pos + sizeof(T) <= Size(), string.Format("Attempted to put value with size: {0} in ByteBuffer (pos: {1} size: {2})", sizeof(T), pos, Size()));
        ...
        {
            Debug.Assert(pos >= 0, string.Format("Attempted to read value with invalid pos: {0} in ByteBuffer", pos));
